    ...
    for (size_t sh = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        for (size_t j = 0; j < total_len; ++j) {
            ...
            for (size_t dim = 0; dim < d; ++dim) {
                dot += static_cast<float>(q[q_base + dim])
                     * static_cast<float>(k[(j * nkvhead + kh) * d + dim]);
            }
            scores[score_base + j] = dot * scale;
        }
    }
}
    ...
    for (size_t sh = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        for (size_t dim = 0; dim < dv; ++dim) {
            ...
            for (size_t j = 0; j < total_len; ++j) {
                ...
                if (sc > 0.0f) {
                    val += sc * static_cast<float>(
                        v[(j * nkvhead + kh) * dv + dim]);
                }
            }
            attn_val[out_base + dim] = static_cast<T>(val);
        }
    }
}
...
void self_attention(std::byte *attn_val, const std::byte *q,
    ...
    CUDA_CHECK(cudaMalloc(&scores_dev, scores_bytes));
    ...
    case LLAISYS_DTYPE_BF16:
#if __CUDACC_VER_MAJOR__ >= 11
        qk_scores_kernel<__nv_bfloat16><<<GRID, BLOCK>>>(
            scores_dev,
        ...
        break;
#else
        CUDA_CHECK(cudaFree(scores_dev));
    ...
    case LLAISYS_DTYPE_BF16:
#if __CUDACC_VER_MAJOR__ >= 11
        combine_kernel<__nv_bfloat16><<<GRID, BLOCK>>>(
            reinterpret_cast<__nv_bfloat16 *>(attn_val), scores_dev,
