...
// ---------------------------------------------------------------------------
// MUSA kernel: multi-block partial argmax reduction
// ---------------------------------------------------------------------------
 ...
 *
 * @tparam T          Element type (float, __half, __mt_bfloat16).
...
// ---------------------------------------------------------------------------
// MUSA kernel: final reduction over partial results
// ---------------------------------------------------------------------------
...
void argmax(std::byte *max_idx, std::byte *max_val, const std::byte *vals,
    ...
    case LLAISYS_DTYPE_F32: {
        ...
        MUSA_CHECK(musaMalloc(&partial_idxs, grid * sizeof(int64_t)));
        MUSA_CHECK(musaMalloc(&partial_vals, grid * sizeof(float)));
        ...
        MUSA_CHECK(musaFree(partial_vals));
        MUSA_CHECK(musaFree(partial_idxs));
        break;
    }
    ...
    case LLAISYS_DTYPE_F16: {
        ...
        int64_t *partial_idxs;
        MUSA_CHECK(musaMalloc(&partial_idxs, grid * sizeof(int64_t)));
        MUSA_CHECK(musaMalloc(&partial_vals, grid * sizeof(__half)));
...
#ifdef __MUSACC__
    {
        __mt_bfloat16 *partial_vals;
        int64_t         *partial_idxs;
        MUSA_CHECK(musaMalloc(&partial_idxs, grid * sizeof(int64_t)));
        MUSA_CHECK(musaMalloc(&partial_vals, grid * sizeof(__mt_bfloat16)));

        argmax_partial_kernel<__mt_bfloat16, BLOCK><<<grid, BLOCK>>>(
            reinterpret_cast<const __mt_bfloat16 *>(vals),
            numel, chunk_size,
            partial_idxs, partial_vals);

        argmax_final_kernel<__mt_bfloat16, BLOCK><<<1, BLOCK>>>(
            partial_idxs, partial_vals, grid,
            reinterpret_cast<int64_t *>(max_idx),
